...
def znajdz_synonimy(slowo: str) -> list[str] | str | None:  # type: ignore[return]
    """
    Zwraca lista synonimów dla podanego słowa.
...
def usun_slowo(slowo: str) -> None:
    ...
    if slowo in baza_synonimow:
        ...
        print(f"  Słowo '{slowo}' zostało pomyślnie usunięte z bazy.")
    else:
...
def wyswietl_statystyki() -> None:
    ...
    print(f"    Liczba słów w bazie    :  {liczba_slow:>2}")
    print(f"    Łączna liczba synonimów:  {laczna_liczba_synonimow:>2}")
