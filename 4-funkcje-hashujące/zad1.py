...
class ProstaMapa:
    ...
    def _funkcja_hashujaca(self, klucz: str) -> int:
        ...
        kody_ascii = [ord(znak) for znak in klucz]
        suma_kodow = sum(kody_ascii)
        return suma_kodow % self.rozmiar
    ...
    def wstaw(self, klucz: str, wartosc) -> None:
        ...
        kubelek = self.kubełki[indeks]

        for i in range(len(kubelek)):
            if kubelek[i][0] == klucz:
                kubelek[i] = (klucz, wartosc)
                return

        kubelek.append((klucz, wartosc))
    ...
    def pobierz(self, klucz: str):
        ...
        Returns:
            wachtość przypisana do klucza lub komunikat o jego braku
        """
        # TODO: 1. Oblicz indeks kubełka używając self._funkcja_hashujaca(klucz).
        ...
        #          zwrć napis: f"Klucz '{klucz}' nie istnieje w mapie."
        indeks = self._funkcja_hashujaca(klucz)
        kubelek = self.kubełki[indeks]
