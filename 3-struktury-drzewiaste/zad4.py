...
def dodaj_zgloszenie(kolejka: list, priorytet: int, id_zgloszenia: int, opis: str) -> None:
    ...
    element = (priorytet, id_zgloszenia, opis)
    heapq.heappush(kolejka, element)
    print("Dodano zgłoszenie ID " + str(id_zgloszenia) + " [Priorytet " + str(priorytet) + "]: '" + opis + "'")
...
def obsluz_zgloszenie(kolejka: list) -> Zgloszenie | None:
    ...
    if len(kolejka) == 0:
        print("Brak zgłoszeń do obsłużenia. Kolejka jest pusta.")
        return None

    element = heapq.heappop(kolejka)
    priorytet = element[0]
    id_zgloszenia = element[1]
    opis = element[2]

    print("Obsłużono zgłoszenie ID " + str(id_zgloszenia) + " [Priorytet " + str(priorytet) + "]: '" + opis + "'")
    return element
...
def wyswietl_kolejke(kolejka: list) -> None:
    ...
    print("Aktualna lista zgłoszeń (" + str(len(kolejka)) + " szt.) w kolejności ważności:")
    kopia_kolejki = sorted(kolejka)
    for element in kopia_kolejki:
        priorytet = element[0]
        id_zgloszenia = element[1]
        opis = element[2]
        print("  [Prio: " + str(priorytet) + "] ID: " + str(id_zgloszenia) + " - " + opis)
