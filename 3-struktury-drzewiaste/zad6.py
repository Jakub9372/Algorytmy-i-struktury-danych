...
def scalaj_logi(*strumienie: list[LogEntry]) -> list[LogEntry]:
    ...
    generator_wynikowy = heapq.merge(*strumienie)
    lista_wynikowa = list(generator_wynikowy)
    liczba_strumieni = len(strumienie)
    liczba_wpisow = len(lista_wynikowa)
    print("Scalono " + str(liczba_strumieni) + " strumieni. Łącznie wpisów: " + str(liczba_wpisow))
...
def filtruj_bledy(logi: list[LogEntry], poziom: str) -> list[LogEntry]:
    ...
    but często używamy tej funkcji razem ze scalonymi logami.
    ...
    print("Znaleziono wpisów: " + str(len(przefiltrowane)))
