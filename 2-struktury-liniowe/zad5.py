...
#     3. Cofnięcie ostatnią operację (pop ze stosu).
...
def wyswietl_stos() -> None:
    ...
    total = len(historia_operacji)
    for i, operacja in enumerate(reversed(historia_operacji), start=1):
        numer = total - i + 1
        if i == 1:
            print(f"[TOP]  {numer}. {operacja}   ← (ostatnia)")
        elif i == total:
            print(f"[BOT]  {numer}. {operacja}")
        else:
            print(f"       {numer}. {operacja}")
